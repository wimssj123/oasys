...
public class IpUtils {
     ...
     */
    public static JsonIp getAddresses(String content, String encodingString)
            throws UnsupportedEncodingException {
        // 这里调用淘宝API
        ...
        if (returnStr != null) {
            ...
            if(temp.length<3){
                return new JsonIp(1,null);//无效IP，局域网测试
            }

            ObjectMapper mapper = new ObjectMapper();
            JsonIp ip = new JsonIp();
            try {
                ip = mapper.readValue(returnStr, JsonIp.class);
